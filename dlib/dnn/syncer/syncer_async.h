...
namespace dlib
{
	...
	template <typename trainer_type,
			  typename data_type,
			  typename label_type>
	int dnn_full_leader<trainer_type, data_type, label_type>::receive_gradients_from_one(int slave_index, std::vector<resizable_tensor> &cli_tensors)
	{
		...
		if (this->me.sync_type == device_sync_type::sync) {
			...
			while (1)
			{
				...
				for (int i = 0; i < sizeof(children_request_tasks)/sizeof(task_op); i++)
				{
					...
					if(this->verbose) ::cout << worker_job.opcode << ":" << worker_job.operand1 << "~" << worker_job.operand2 << std::endl;
