...
struct device
{
	...
	device(int number_, std::string ip_, int port_, int role_, int master_, int comp_)
		: number(number_), ip(ip_), port(port_), role(role_), master(master_), comp_ability(comp_)
	{
	}
